...
in
{
  options.services.proximity_matcher_webservice = {
    ...
    package = lib.mkOption {
      ...
      default = pkgs.proximity_matcher_webservice;
  ...
  config = {
    systemd.services.proximity_matcher_webservice = lib.mkIf cfg.enable {
      ...
      serviceConfig = {
        ExecStart = "${cfg.package.python}/bin/gunicorn -w ${toString cfg.workers} -b ${cfg.listenAddress}:${toString cfg.listenPort} --preload${lib.optionalString cfg.optimized " -t ${toString cfg.timeout}"} proximity_matcher_webservice.proximity_server${lib.optionalString cfg.optimized "_opt"}:app";
