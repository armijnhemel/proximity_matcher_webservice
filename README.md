...
The server needs to load data to be useful. This data should be in pickle form
and can be generated by `create_vpt_pickle.py` as described above.

The location of the pickle can be set through a configuration file. The
location of the configuration file in turn can be set through an environment
variable.

An example configuration file can be found in the file `proximity_config.py`.
Currently it looks like this:

```
TLSH_PICKLE_FILE = '/tmp/licenses-tlsh.pickle'
KNOWN_TLSH_HASHES = '/tmp/hashes.txt'
```

`TLSH_PICKLE_FILE` points to the pickle file with TLSH hashes as created by
`create_vpt_pickle.py`.

`KNOWN_TLSH_HASHES` is only used by the optimized version of the server and is
a file with one TLSH hash per line. These are the known hashes which are cached
to avoid tree traversals (see below).

The location of the configuration is set by the environment variable
`PROXIMITY_CONFIGURATION`, for example:

```console
export PROXIMITY_CONFIGURATION=/tmp/proximity_config.py
```

### Flask

```
$ export PROXIMITY_CONFIGURATION=/path/to/config
